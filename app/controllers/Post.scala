...
object Post extends Controller with Security {
  ...
  def list(page: Long) = OnlyAuthenticated(Privilege.admin) {
    user =>
      implicit request =>
  ...
  def create = OnlyAuthenticated(Privilege.admin, Privilege.editor, Privilege.writer) {
    user =>
      implicit request =>
        Ok(html.post.create(postForm))
  }

  def edit(id: Long) = OnlyAuthenticated(Privilege.admin, Privilege.editor, Privilege.writer) {
    user =>
      implicit request =>
        models.Post.findById(id) match {
  ...
  def processPostForm(id: Long) = OnlyAuthenticated(Privilege.admin, Privilege.editor, Privilege.writer) {
    user =>
      implicit request =>
